...
#endif
            size_t size = stream->parsePacket();
            if (size > PACKET_SIZE)
                size = PACKET_SIZE;
...
#ifdef ARTNET_ENABLE_UNIX

        // TODO
        template <typename T = S> IPAddress localIP() {
            IPAddress res;
            res.fromString("127.0.0.1");
            return res;
        }
        template <typename T = S> IPAddress subnetMask() {
            IPAddress res;
            res.fromString("255.255.255.0");
            return res;
        }
        template <typename T = S> auto macAddress(uint8_t *mac) { // TODO
            mac[0] = 50;
            return;
        }
#endif // ARTNET_ENABLE_UNIX
